...
export const getArtifactForWorkflowRun = async ({
	...
	const cacheKey = `https://prerelease-registry.onlineornot.workers.dev/runs/${runID}/${name}`;
	...
	try {
		const artifactsResponse = await gitHubFetch(
			`https://api.github.com/repos/onlineornot/onlineornot/actions/runs/${runID}/artifacts`,
			{
				headers: {
