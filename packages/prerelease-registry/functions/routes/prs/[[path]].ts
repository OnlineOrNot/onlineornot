...
> = async ({ params, env, waitUntil }) => {
	...
	try {
		const pullRequestsResponse = await gitHubFetch(
			`https://api.github.com/repos/onlineornot/onlineornot/pulls/${pullRequestID}`,
			{
				headers: {
		...
		const workflowRunsResponse = await gitHubFetch(
			`https://api.github.com/repos/onlineornot/onlineornot/actions/runs?branch=${branch}&per_page=100&event=pull_request`,
			{
				headers: {
					Accept: "application/vnd.github.v3+json",
				},
